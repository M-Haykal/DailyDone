                    ...
                    <ul class="nav nav-tabs nav-fill" id="userTab{{ $shared->id }}" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="profile-tab-{{ $shared->id }}" data-bs-toggle="tab" 
                                data-bs-target="#profile-{{ $shared->id }}" type="button" role="tab">Profile</button>
                        </li>
                        @if(auth()->id() == $project->user_id || auth()->user()->can('edit', $project))
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="permission-tab-{{ $shared->id }}" data-bs-toggle="tab" 
                                    data-bs-target="#permission-{{ $shared->id }}" type="button" role="tab">Permissions</button>
                            </li>
                        @endif
                    </ul>
                    ...
                    <div class="tab-content pt-3" id="userTabContent{{ $shared->id }}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $shared->user->role }}" readonly>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Joined</label>
                                        <input type="text" class="form-control" value="{{ $shared->user->created_at->format('d M Y') }}" readonly>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Project Permission</label>
                                        <input type="text" class="form-control" value="{{ ucfirst($shared->permissions) }}" readonly>
                                    </div>
                                </div>
                            </div>  
                        </div>
                        ...
                        @if(auth()->id() == $project->user_id || auth()->user()->can('edit', $project))
                            <div class="tab-pane fade" id="permission-{{ $shared->id }}" role="tabpanel">
                                <form action="{{ route('user.project.editPermission', ['project' => $project->id, 'sharedProject' => $shared->id]) }}" method="POST">
                                    @csrf
                                    @method('PUT')

                                    <div class="mb-3">
                                        <label class="form-label">Email</label>
                                        <input type="email" class="form-control" value="{{ $shared->user ? $shared->user->email : $shared->email }}" readonly>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label">Permission Level</label>
                                        <select name="permissions" class="form-select">
                                            <option value="view" {{ $shared->permissions == 'view' ? 'selected' : '' }}>Can View</option>
                                            <option value="edit" {{ $shared->permissions == 'edit' ? 'selected' : '' }}>Can Edit</option>
                                        </select>
                                    </div>

                                    <div class="d-flex justify-content-between">
                                        <button type="button" class="btn btn-danger" onclick="confirmDeleteAccess('{{ $shared->id }}')">
